...
description: "How Acme Analytics measures the impact of advertising"
icon: "chart-line"
---

## What is lift measurement?

Lift measurement quantifies the incremental impact of advertising on consumer perception. By comparing survey responses from people who saw an ad (exposed group) versus people who didn't (control group), the platform calculates how much a campaign moved key metrics.

## Key metrics

<CardGroup cols={2}>
  <Card title="Awareness" icon="eye">
    "Have you heard of this brand?" Measures whether advertising increased
    the percentage of people who recognize the brand.
  </Card>
...
graph TD
    ...
    D --> F[Compare Results]
    E --> F
    F --> G[Lift = Exposed - Control]
```

The core methodology:

1. **Exposure tracking**: The Data Collector records which users were served an ad impression via ad server integration.
2. **Survey distribution**: Both exposed and control users receive identical brand perception surveys.
3. **Statistical analysis**: The Analysis Engine computes the difference in positive response rates between groups, applying significance testing.

### Statistical significance

The platform uses hypothesis testing to ensure reported lift is real, not noise.
...
<CardGroup cols={2}>
  <Card title="Campaigns" icon="bullhorn" href="/guides/campaigns">
    Set up and manage measurement studies.
  </Card>
